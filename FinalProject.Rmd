...
##### c) RM ANOVA Summary per proteins
...
for (prot in unique(df_subset$Protein)) {
  ...
  # Try RM ANOVA and extract full table
  tryCatch({
    model <- aov(Abundance ~ Condition + Error(BioReplicate/Condition), data = temp)
    summary_out <- summary(model)
...
### 3) Linear Mixed Effects Model (LMM) & 4) Linaer Model (LM)
...
# Step 2: Create label of significant contrasts per protein
